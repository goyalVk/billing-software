...
export default function SupplierDetail() {
  ...
  async function markPaid(purchaseId) {
    setMarkingId(purchaseId);
    try {
      await api.put(`/purchases/${purchaseId}/mark-paid`, {
        ...(payDateInputs[purchaseId] && { date: payDateInputs[purchaseId] }),
      });
      await load();
    } catch (err) {
  ...
  async function recordPayment(purchaseId) {
    const amount = Number(payInputs[purchaseId]);
    if (!(amount > 0)) {
    ...
    setMarkingId(purchaseId);
    try {
      await api.put(`/purchases/${purchaseId}/record-payment`, {
        amount,
        ...(payDateInputs[purchaseId] && { date: payDateInputs[purchaseId] }),
      });
      setPayInputs((prev) => ({ ...prev, [purchaseId]: "" }));
      await load();
    } catch (err) {
  ...
  const { supplier, purchases, totalPurchases, totalPaid, totalDue } = data;
  const displayedPurchases = showOnlyDues
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5 mb-6">
        <div className="flex items-start justify-between flex-wrap gap-4 mb-4">
          <div>
            ...
            {supplier.address && <p className="text-gray-500 text-sm">{supplier.address}</p>}
          </div>
          <button
            onClick={() =>
              navigate("/inventory", {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <p className="text-xs text-gray-500">Total Due to Supplier</p>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {displayedPurchases.map((p) => {
              const amountPaid = p.amountPaid ?? p.totalAmount;
              const remaining = p.totalAmount - amountPaid;
              const isDue = p.paymentStatus !== "paid";
              const hasPayments = p.payments && p.payments.length > 0;
              return (
                <Fragment key={p._id}>
                  <tr className={`border-t border-gray-100 ${isDue ? "bg-red-50" : ""}`}>
                    ...
                    <td className="px-4 py-3">
                      {p.dueDate ? (
                        <span
                          className={isOverdue(p.dueDate) ? "text-red-600 font-semibold" : "text-gray-600"}
                        >
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2 flex-wrap">
                        {isDue && (
                          <>
                            <input
                              ...
                              onChange={(e) =>
                                setPayInputs((prev) => ({ ...prev, [p._id]: e.target.value }))
                              }
                            ...
                            <input
                              ...
                              min={toLocalDateTimeInputValue(p.date)}
                              max={nowDateTimeLocal()}
                              value={payDateInputs[p._id] || ""}
                              onChange={(e) =>
                                setPayDateInputs((prev) => ({ ...prev, [p._id]: e.target.value }))
                              }
                              className="w-48 border border-gray-300 rounded-md px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            />
                            <button
                              onClick={() => recordPayment(p._id)}
                        ...
                        {hasPayments && (
                          ...
                          >
                            {expandedId === p._id ? "Hide History" : "Payment History"}
                          </button>
                        )}
                      </div>
                    </td>
                  </tr>
                  {expandedId === p._id && hasPayments && (
                    <tr className="bg-gray-50 border-t border-gray-100">
                      <td colSpan={9} className="px-4 py-3">
                        ...
                        <table className="text-sm">
                          ...
                          <tbody>
                            {[...p.payments].reverse().map((pay, i) => (
                              <tr key={i} className="border-t border-gray-200">
                                <td className="pr-6 py-1">{new Date(pay.date).toLocaleString()}</td>
                                <td className="pr-6 py-1 text-right">Rs. {pay.amount.toFixed(2)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
